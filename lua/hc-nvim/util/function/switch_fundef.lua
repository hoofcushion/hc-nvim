...
local function find_assignment(node)
 local found=pcall(function()
  local proxy=HCNvim.Util.TSProxy.new(node)
...
local function switch(buf,win)
 ...
 try(function()
  ...
  local assign_node=HCNvim.Util.ts_lookup(cursor_node,find_assignment)
  local declare_node=HCNvim.Util.ts_lookup(cursor_node,"function_declaration")
  if assign_node and declare_node then
   local cursor=vim.api.nvim_win_get_cursor(win)
   local pos={cursor[1]-1,cursor[2]}
   local assign_range={assign_node:range()}
   local a_distance=HCNvim.Util.ts_get_distance(pos,assign_range)
   local declare_range={declare_node:range()}
   local d_distance=HCNvim.Util.ts_get_distance(pos,declare_range)
  ...
  if assign_node then
   try(function()
    local proxy=HCNvim.Util.TSProxy.new(assign_node)
  ...
  if declare_node then
   try(function()
    local proxy=HCNvim.Util.TSProxy.new(declare_node)

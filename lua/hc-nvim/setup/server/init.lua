...
local lspMaps; lspMaps=Util.lazy(function()
 ---@param cap vim.lsp.protocol.Methods
 ...
 Mapping:extend({
  ...
  {name=NS.lsp_inlay_hints_toggle,     rhs=function() vim.lsp.inlay_hint.enable(not vim.lsp.inlay_hint.is_enabled()) end,cond=has_cap("textDocument/inlayHint")},
  {
   ...
   rhs=function()
    ...
    local enabled=Util.tbl_get(client,{"config","settings","Lua","hint","enable"})
    local target=not enabled
    Util.tbl_set(client,{"config","settings","Lua","hint","enable"},target)
    local buf=vim.api.nvim_get_current_buf()
